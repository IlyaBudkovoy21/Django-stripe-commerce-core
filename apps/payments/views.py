...
def create_item_intent(request, item_id: int):
    if request.method != "GET":
        return JsonResponse({"error": "Method not allowed"}, status=405)

    item = get_object_or_404(Item, pk=item_id)
    try:
        intent = create_item_payment_intent(item_id=item.id, amount=item.price, currency=item.currency)
    except Exception as exc:
        return JsonResponse({"error": str(exc)}, status=400)
...
def create_order_intent(request, order_id: int):
    if request.method != "GET":
        return JsonResponse({"error": "Method not allowed"}, status=405)

    order = get_object_or_404(Order, pk=order_id)
    try:
        intent = create_order_payment_intent(order=order)
    except Exception as exc:
        return JsonResponse({"error": str(exc)}, status=400)
